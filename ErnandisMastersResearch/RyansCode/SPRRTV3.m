clear variables;
NumTrials = 50;
Data(NumTrials).Path.IT = nan(1000,1);
Data(NumTrials).Err.IT = cell(1000,1);
Data(NumTrials).Time = NaN(1000,1);
Data(NumTrials).NumNodes = NaN(1000,1);
set(0,'DefaultFigureVisible','off');
for TN = 1:NumTrials
    clearvars -except TN Data NumTrials
    close all;
    dbstop if error
    ...
    if problemDefinition == 1
        ...
        ballConstant = 10000; % Affects the size of the relative neighborhood
        
        
        ErrFun = @VolEllipsoid;
        % ErrFun = @(PMat) trace(PMat);
        steer = @SPSteerV2; % steering function to use
        ...
        QMat = [Poserr*eye(3) zeros(3); zeros(3) Angerr*eye(3)];
    ...
    elseif problemDefinition == 2
        ...
        QMat = 5*[0.0844 -5.0781e-5; -5.0781e-5 0.0844]; % Process Noise Covariance
    ...
    while ~stopingCriteriaMet(tcur)
        ...
        [localTrajectory, newPMat] = steer(closestNode.position', randPoint', G.delta,W,Tol, closestNode.PMat, QMat);
        ...
        elseif algorithmToUse == 1
            ...
            for n_i = 1:numNeighbors
                ...
                elseif neighborNode.err < errFromStartNewPoint
                    ...
                    [neighborLocalTrajectory,neighborPMat] = steer(neighborNode.position', newPoint', delta,W,Tol,neighborNode.PMat, QMat);
        ...
        elseif algorithmToUse == 2
            ...
            for n_i = 1:numPredecessors
                ...
                if closestNode.id == predecessorNode.id
                    ...
                    [predecessorLocalTrajectory,predecessorPMat] = steer(predecessorNode.position', newPoint', delta,W,Tol, predecessorNode.PMat, QMat);
        ...
        if algorithmToUse == 1
            ...
            for n_i = 1:length(kNN)
                ...
                if newNode.err < neighborNode.err
                    ...
                    [neighborLocalTrajectory,neighborPMat] = steer(newNode.position', neighborNode.position', delta, W, Tol, newNode.PMat, QMat);
                    if isempty(neighborLocalTrajectory)
