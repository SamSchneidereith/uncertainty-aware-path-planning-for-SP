clearvars -except startPosition goalPosition
NumTrials = 1;
...
Count = 0;
for TN = 1:NumTrials
    clearvars -except TN Data NumTrials startPosition goalPosition Count
    close all;
    dbstop if error
    ...
    problemDefinition = 1;  % 1: Stewart Platform [x,y,z,Xrot,Yrot,Zrot]
                            % 2: 2D Example [x,y]
    %------------------------- BEGIN USER INPUT ------------------------------%
    ...
    if problemDefinition == 1
        ...
        ballConstant = 2640; % Affects the size of the relative neighborhood
        ...
        QMat = [Poserr*eye(3) 1e-6*ones(3); 1e-6*ones(3) Angerr*eye(3)];
    ...
    elseif problemDefinition == 2
        ...
        QMat = 1*[0.0844 -5.0781e-5; -5.0781e-5 0.0844]; % Process Noise Covariance
    ...
    while ~stopingCriteriaMet(tcur)
        ...
        [localTrajectory, newPMat] = steer(closestNode.position', randPoint', G.delta,W,Tol, closestNode.PMat, QMat,1);
        ...
        elseif algorithmToUse == 1
            ...
            for n_i = 1:numNeighbors
                ...
                elseif neighborNode.err < errFromStartNewPoint
                    ...
                    [neighborLocalTrajectory,neighborPMat] = steer(neighborNode.position', newPoint', delta,W,Tol,neighborNode.PMat, QMat,1);
        ...
        elseif algorithmToUse == 2
            ...
            for n_i = 1:numPredecessors
                ...
                if closestNode.id == predecessorNode.id
                    ...
                    [predecessorLocalTrajectory,predecessorPMat] = steer(predecessorNode.position', newPoint', delta,W,Tol, predecessorNode.PMat, QMat,1);
        ...
        if algorithmToUse == 1
            ...
            for n_i = 1:length(kNN)
                ...
                if newNode.err < neighborNode.err
                    ...
                    [neighborLocalTrajectory,neighborPMat] = steer(newNode.position', neighborNode.position', delta, W, Tol, newNode.PMat, QMat,1);
                    if isempty(neighborLocalTrajectory)
    ...
    if TakeShot == 1
        ...
        zlabel('Z')
        title('Posistion')
        filename = [pwd,'\RRTsharp',num2str(Count),'FullTree.png'];
        saveas(fig,filename);
        ...
        filename = [pwd,'\RRTsharp',num2str(Count),'PathTree.png'];
        saveas(fig,filename);
...
%set(0,'DefaultFigureVisible','on');
    ...
    if problemDefinition == 1
        fileloc = [pwd,'\Plots\SP\',date,'\'];
        if ~isfolder(fileloc)
            mkdir(fileloc)
        end
    elseif problemDefinition == 2
        fileloc = [pwd,'\Plots\2D\',date,'\'];
        if ~isfolder(fileloc)
            mkdir(fileloc)
        end
    end
...
%%
    ...
    %SaveParaData(G,TN,QMat,problemDefinition)
    %SaveFullData(G,TN,problemDefinition)
